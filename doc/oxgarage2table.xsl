...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="serverToUse"
        >https://meigarage.edirom.de/ege-webservice/</xsl:param>
    <xsl:param name="garageName"
        >OxGarage</xsl:param>
    <xsl:variable name="server" select="$serverToUse"/>
    <xsl:variable name="conversions"
        select="document(concat($server,'Conversions/'))"/>
    ...
    <xsl:template name="main">
        <TEI
            ...
            <teiHeader>
                <fileDesc>
                    <titleStmt>
                        <title><xsl:value-of select="$garageName"/> Conversion Matrix</title>
                    </titleStmt>
                    <publicationStmt>
